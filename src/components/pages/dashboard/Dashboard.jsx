...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      await Promise.all([fetchUsers(), fetchPendingLoans(), fetchApprovedLoans()]);
    } catch (error) {
  ...
  const fetchPendingLoans = async () => {
    try {
      const loansQuery = query(collection(db, "applications"), where("status", "==", "pending"));
      const loansSnapshot = await getDocs(loansQuery);
      const pendingLoansList = loansSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  ...
  const fetchApprovedLoans = async () => {
    try {
      ...
      const approvedLoansList = loansSnapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  ...
  const totalDisbursedAmount = approvedLoans.reduce((sum, loan) => sum + loan.amount, 0);
  ...
  return (
    <Box display="flex" minHeight="100vh" bgcolor="#f5f5f5">
      {/* Left Sidebar */}
      <Paper elevation={3} sx={{ width: "250px", minHeight: "100vh", bgcolor: "#1e293b", color: "white" }}>
        <Box p={2}>
          ...
          <List>
            <ListItem button onClick={() => setActiveTab("users")} sx={{ bgcolor: activeTab === "users" ? "#334155" : "transparent" }}>
              <ListItemIcon>
                <UsersIcon sx={{ color: "white" }} />
              </ListItemIcon>
              <ListItemText primary="Users" />
            </ListItem>
            <ListItem button onClick={() => setActiveTab("loans")} sx={{ bgcolor: activeTab === "loans" ? "#334155" : "transparent" }}>
              <ListItemIcon>
                <LoansIcon sx={{ color: "white" }} />
              </ListItemIcon>
              <ListItemText primary="Loan Applications" />
            </ListItem>
            <ListItem button onClick={() => setActiveTab("payments")} sx={{ bgcolor: activeTab === "payments" ? "#334155" : "transparent" }}>
              <ListItemIcon>
                <PaymentsIcon sx={{ color: "white" }} />
              </ListItemIcon>
              <ListItemText primary="Payments" />
            </ListItem>
            <ListItem button onClick={() => setActiveTab("notifications")} sx={{ bgcolor: activeTab === "notifications" ? "#334155" : "transparent" }}>
              <ListItemIcon>
                <NotificationsIcon sx={{ color: "white" }} />
              </ListItemIcon>
              <ListItemText primary="Notifications" />
            </ListItem>
            <ListItem button onClick={() => setActiveTab("approved")} sx={{ bgcolor: activeTab === "approved" ? "#334155" : "transparent" }}>
              <ListItemIcon>
              ...
              <List>
                {pendingLoans.map((loan) => (
                  <ListItem key={loan.id}>
                    <ListItemText primary={`User: ${loan.userId}`} secondary={`Amount: ₹${loan.amount}`} />
                  </ListItem>
                ))}
              </List>
            </>
